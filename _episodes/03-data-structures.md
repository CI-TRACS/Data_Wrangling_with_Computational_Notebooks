...
When creating a `Series` pandas will store all the data as the same type. The mapping from the native python types to what they would be in pandas is summarized below. **Note that the `datetime64` type will not be discussed here.**
...
However, when parsing plain text files can become a complicated procedure. To aid in the process, pandas provides a lot of optional parameters that may be set when calling the `read_csv()` function. To learn more, see the `read_csv()` [documentation](https://pandas.pydata.org/docs/reference/api/pandas.read_csv.html), which summarizes all of the parameters. We will be covering many of the most important parameters as we work through this lesson.
...
![No Headers Dataframe](../fig/E3_2_no_columns_dataframe.png)
